...
for i in {1..3}; do
    ...
    if [ -f "$PRO_FILE" ]; then
        ...
        echo "Se está por crear el archivo $OUTPUT_DIR" | tee -a "$LOG_FILE"
        sudo mkdir -p "$OUTPUT_DIR"
        echo "Carpeta creada $OUTPUT_DIR..." | tee -a "$LOG_FILE"
        ...
        sudo mv *.pos *.res *.pre *.log *.txt "$OUTPUT_DIR" 2>/dev/null
